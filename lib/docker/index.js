...
async function writeDockerfile(dir, modules, config) {
	const compiled = _.template(await fs.readFileAsync(path.join(__dirname, getTemplateType(modules))))
	...
	await fs.writeFileAsync(packageJsonPath, JSON.stringify(conf.packageJson, null, '\t'))

	await fs.writeFileAsync(dockerfilePath, compiled(conf))
