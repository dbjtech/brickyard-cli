...
class Brickyard extends events.EventEmitter {
	...
	scanAsync(...inputPaths) {
		const paths = _.map(inputPaths, getPath)
	...
	buildConfigAsync(configfile) {
		return Bluebird.coroutine(function* g() {
			...
			const config = tryToRequire(getPath(configfile))
			...
			yield fs.writeFileAsync(`${this.dirs.dest}/setting.js`, `module.exports = ${JSON.stringify(setting, null, '\t')}`)
		}).bind(this)()
	}

	saveAsync(dir, config) {
		return Bluebird.coroutine(function* g() {
			this.setBuildPath(dir)

			yield this.buildModules()
			yield this.buildConfigAsync(config)
		}).bind(this)()
	}
	...
	loadAsync(dir) {
		return Bluebird.coroutine(function* g() {
			...
			yield fs.accessAsync(settingPath)
