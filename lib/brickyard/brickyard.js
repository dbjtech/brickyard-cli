...
class BrickyardModules {
	constructor(_package_json) {
		...
		this.path = path.dirname(package_json).replace(/\\/g, '/')
		...
		if (/^buildtask/i.test(config.description)) {
			this.type = 'buildtask'
...
class Brickyard {
	...
	scan(..._paths) {
		...
		let files = glob.sync(`${this.modulePaths}/**/package.json`)
	...
	prepareModule() {
		// 1, 2
		for (let expectModule of this.expectModules) {
			for (let md of this.allModules) {
				let pattern1 = `${this.modulePaths}/${expectModule}`
				let pattern2 = `${this.modulePaths}/${expectModule}/**`
	 ...
	 */
	loadBuildTasks() {
		module.require_alias('brickyard', __dirname)
		_.each(this.modules.buildtask, (md) => {
			console.debug('load', md.name)
			require(`brickyard/${md.name}`)
		})
	}
	/*
	...
	setConfig(configfile) {
		...
		module.require_alias('brickyard/config', abs)
	...
	dumpConfig() {
		this.config.modules = []
		for (let md of this.modules) {
			if (this.modules.backend[md.name] || this.modules.buildtask[md.name]) {
