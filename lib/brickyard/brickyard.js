...
function alias_brickyard_module(base, md) {
	...
	let mainDestFullPath = `${path.join(base, md.type, md.name)}`
...
class Brickyard {
	...
	loadBuildTasks() {
		...
		_.each(this.modules.buildtask, (md) => {
			try_to_require(`brickyard/${md.type}/${md.name}`)
		})
	}
	/*
	...
	isBuilt() {
		...
		let getModuleNameFromPath = (pattern) =>
			_.map(glob.sync(`${pattern}/*`), (v) => path.relative(`${pattern}`, v))


		for (let type of ['plan', 'buildtask', 'backend']) {
			let modules = getModuleNameFromPath(`${dir}/brickyard_modules/${type}`)
			let xor = _.xor(modules, _.keys(this.modules[type]))
			// console.trace(type, modules, xor)
			if (xor.length) return false
		}
