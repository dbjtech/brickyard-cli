...
const brickyardRunSignals = ['before-run', 'run', 'after-run']
	...
	return new Promise((resolve, reject) => {
		fork().on('message', (msg) => {
			if (msg === BRICKYARD_CHILD_PROCESS_RUN_AGAIN) {
				forkAndWaitMsg()
			} else if (msg === BRICKYARD_CHILD_PROCESS_BUILD_FINISHED) {
				resolve()
			} else {
				reject()
			}
...
gulp.task('build', (() => async () => {
	const { dir, config, run, watch } = argv
	if (isMaster) { // master fork and run and watch
		await forkAndWaitMsg()
		if (watch) {
			await brickyard.run(argv, 'watch-output')
		}
	} else { // worker build
		await runSequence('init')
		...
		let signals = []
		if (watch) {
			signals = _.concat(signals, 'watch-frontend', 'watch-backend')
		}
		if (run) {
			signals = _.concat(signals, brickyardRunSignals)
			brickyard.loadSettingAsync(dir)
			brickyard.loadModules('backend')
		}
		brickyard.sendSignals(...signals)
	}
})())
...
gulp.task('run', (() => async () => {
	...
	if (instances === 1) {
		await brickyard.run(argv, ...brickyardRunSignals)
		return
