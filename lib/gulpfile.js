...
gulp.task('build', ['init'], () => Bluebird.coroutine(function* g() {
	...
	_.each(brickyard.modules, (md) => {
		const pushStream = (...pathName) => {
			streams.push(new Bluebird((resolve, reject) => {
				gulp.src([`${md.path}/**`])
					.pipe(gulp.dest(path.join(...pathName, md.name)))
					.on('end', (err) => {
		...
		if (['buildtask', 'backend', 'plan'].indexOf(md.type) !== -1) {
			pushStream(brickyard.dirs.modules, md.type)
		} else if (md.type === 'frontend') {
			pushStream(brickyard.dirs.tempModules)
		}
	})
...
gulp.task('debug', () => Bluebird.coroutine(function* g() {
	...
	const MASTER_WATCH_FINISHED = 2
	const listen = (event, message) => new Bluebird((resolve) => {
	...
	if (cluster.isMaster) {
		...
		yield listen(worker, CLUSTER_RUN_FINISHED)
		yield runFlatten('watch')
		worker.send(MASTER_WATCH_FINISHED)

		return
	}
	yield runSequence('run')
	const watch = listen(process, MASTER_WATCH_FINISHED)
	process.send(CLUSTER_RUN_FINISHED)
	yield watch
})())

gulp.task('test', ['build'], () => runFlatten('test'))
