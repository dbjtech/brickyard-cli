...
const runFlatten = Bluebird.promisify(gulp.run_flatten)
...
function startDebugCluster() {
	if (cluster.isMaster) {
		...
		worker.once('message', (msg) => {
			...
			return runSequence('run-start').then(() => runFlatten('watch'))
		})
...
gulp.task('ls', () => {
	...
	if (brickyard.isBuilt()) {
		const msg = (argv.plan instanceof Array && argv.plan.length > 1) ?
