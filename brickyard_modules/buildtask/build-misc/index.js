...
gulp.create_tasks({
	...
	build_entry: function () {
		// collect plugins' css
		let paths_css = `${brickyard.dirs.tempModules}/**/*.css`
		let files_css = glob.sync(paths_css)

		for (let path_str of files_css) {
			let split = path.relative(brickyard.dirs.temp, path_str).split(path.sep)
			let id = split[1]
			let plugin = brickyard.modules.frontend[id] || brickyard.bower_plugins[id]
			if (!plugin) {
		...
		return gulp.src(path.join(__dirname, 'main.js'))
			.pipe(gulp.plugins.data(function (/* file */) {
	 ...
	 */
	build_plugins_misc: function () {
		if (brickyard.modules.buildtask['buildtask-webpack-build']) {
			return
		}
		let src = [`${brickyard.dirs.temp}/**/*.{css,map,gif,jpg,jpeg,png,ico,svg,woff,woff2,eot,ttf,xsd,wsdl,mp3,wav}`]
	 ...
	 * @returns {*|Object}
	 */
	build_misc_template_html: function () {
		let template = brickyard.config.misc && brickyard.config.misc.template
		if (_.isEmpty(template)) {
			return
		}

		let streams = []
		_.each(template, function (task, dest) {
			...
			if (task['index-page']) {
				let index_content = fs.readFileSync(path.join(brickyard.dirs.tempModules, task['index-page']))
				p = p.pipe(gulp.plugins.replace(/<!-- index-page -->/, index_content))
			}
	 ...
	 * @returns {*}
	 */
	build_misc_other: function (cb) {
		let others = brickyard.config.misc && brickyard.config.misc.others
		...
		let streams = []
		_.each(others, function (val, dest) {
			let src = others[dest]
			let steam = gulp.src(src)
